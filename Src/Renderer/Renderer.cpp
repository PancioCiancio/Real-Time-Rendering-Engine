...
namespace Renderer {
    ...
    void Renderer::Update(double delta_time)
    {
        ...
        while (stillRunning)
        {
            ...
            const float cameraLerpAlpha = 1.0f - glm::pow(2.0f, -static_cast<float>(deltaTime) / halfTime);
            ...
            vkCmdBindVertexBuffers(framesInFlight.commandBuffer[fifIndex],
                0, 3, &bindsBuffer[0], &OFFSETS[0]);
    ...
    void Renderer::InitInstance()
    {
        ...
        instanceCreateInfo.pNext = &DEBUG_UTILS_MESSENGER_CREATE_INFO; // @TODO: turn it on only if needed (debug build).
    ...
    void Renderer::InitUniformBuffer()
    {
        for (uint32_t i = 0; i < FramesInFlightType::MAX_FIF_COUNT; i++)
        {
            ...
            vk_create_buffer(
                ...
                nullptr,
                &uniformBufferFrames.buffers[i],
                &uniformBufferFrames.memory[i]);
        }
    }
